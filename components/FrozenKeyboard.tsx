      ...
      <group ref={pressRef}>
        ...
        >
          <meshPhysicalMaterial
            ...
            transmission={0}
            roughness={0.32}
            clearcoat={isMobile || lowEnd ? 0 : 0.5}
...
export default function FrozenKeyboard({
  ...
  const [caps, setCaps] = useState<WebGLCaps | null>(null);

  useEffect(() => {
    setCaps(probeWebGL());
  }, []);

  // R3F's Canvas configure() is async and has no .catch(). If WebGL2 still
  // fails after the probe, swallow the rejection and unmount instead of
  // crashing the Next.js error overlay.
  useEffect(() => {
    if (!caps?.ok) return;
    const onReject = (event: PromiseRejectionEvent) => {
      ...
      if (!/WebGL/i.test(msg)) return;
      event.preventDefault();
      setCaps({ ok: false, webgl2: false, lowEnd: true });
    };
    window.addEventListener("unhandledrejection", onReject);
    return () => window.removeEventListener("unhandledrejection", onReject);
  }, [caps?.ok]);

  // Probe runs on the client. Three.js r163+ requires WebGL2, so Intel HD
  // 3000 / ANGLE D3D9 (WebGL1-only) must skip Canvas entirely.
  if (!caps?.ok) return null;

  const lowEnd = caps.lowEnd;
  ...
  const scene = (
    ...
    >
      ...
      {/* Local environment map built from Lightformer quads — no external
          ...
          soft icy highlights without relying on drei's CDN. PMREM needs
          WebGL2, so skip it on D3D9 / HD 3000-class GPUs. */}
      {!lowEnd && (
      ...
      <ambientLight intensity={lowEnd ? 0.45 : 0.15} />
      <directionalLight position={[-5, 8, 3]} intensity={lowEnd ? 1.6 : 2.2} />
      <hemisphereLight
        intensity={lowEnd ? 0.4 : 0.25}
        color="#eaf2fb"
        groundColor="#0a1428"
      />

      <Keyboard mobile={mobile} lowEnd={lowEnd} />

    </Canvas>
  );

  if (!mobile) return scene;
